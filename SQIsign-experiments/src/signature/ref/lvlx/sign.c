...
static void
response_bound(ibz_t *bound, const ibz_t *lattice_content, uint32_t response_length)
{
    ibz_pow(bound, &ibz_const_two, response_length);
    ibz_sub(bound, bound, &ibz_const_one);
    ibz_mul(bound, bound, lattice_content);
}

static void
sample_response_with_length(quat_alg_elem_t *x,
                            ...
                            uint32_t response_length)
{
    ibz_t bound;
    ibz_init(&bound);
    response_bound(&bound, lattice_content, response_length);
...
{
    sample_response_with_length(x, lattice, lattice_content, SQIsign_response_length);
}

static void
compute_response_lattice(quat_lattice_t *lattice_hom_chall_to_com,
                              ...
                              const quat_left_ideal_t *lideal_commit)
{
    ...
    compute_response_lattice(&lattice_hom_chall_to_com, lattice_content, sk, lideal_chall_two, lideal_commit);
...
static void
set_candidate_space(response_isogeny_stats_t *stats, const ibz_vec_4_t *box)
{
    stats->candidate_space = 1;
    stats->candidate_space_overflow = 0;

    for (int i = 0; i < 4; i++) {
        if (ibz_bitsize(&(*box)[i]) > 62) {
            stats->candidate_space = UINT64_MAX;
            stats->candidate_space_overflow = 1;
            return;
        }

        uint64_t half_width = (uint64_t)mpz_get_ui((*box)[i]);
        if (half_width > (UINT64_MAX - 1) / 2) {
            stats->candidate_space = UINT64_MAX;
            stats->candidate_space_overflow = 1;
            return;
        }

        uint64_t width = 2 * half_width + 1;
        if (width != 0 && stats->candidate_space > UINT64_MAX / width) {
            stats->candidate_space = UINT64_MAX;
            stats->candidate_space_overflow = 1;
            return;
        }
        stats->candidate_space *= width;
    }
}

static int
increment_box_vector(ibz_vec_4_t *x, const ibz_vec_4_t *lower, const ibz_vec_4_t *upper)
{
    for (int i = 0; i < 4; i++) {
        if (ibz_cmp(&(*x)[i], &(*upper)[i]) < 0) {
            ibz_add(&(*x)[i], &(*x)[i], &ibz_const_one);
            return 1;
        }
        ibz_copy(&(*x)[i], &(*lower)[i]);
    }
...
static int
response_degree_is_odd(int *is_odd, const quat_alg_elem_t *resp_quat, const ibz_t *lattice_content)
{
    int ok = 0;

    ibz_t remain, degree_full_resp, degree_resp, norm_denom;
    ibz_init(&remain);
    ibz_init(&degree_full_resp);
    ibz_init(&degree_resp);
    ibz_init(&norm_denom);

    quat_alg_norm(&degree_full_resp, &norm_denom, resp_quat, &QUATALG_PINFTY);
    if (!ibz_is_one(&norm_denom))
        goto cleanup;

    ibz_div(&degree_resp, &remain, &degree_full_resp, lattice_content);
    if (!ibz_is_zero(&remain))
        goto cleanup;

    *is_odd = ibz_is_odd(&degree_resp);
    ok = 1;

cleanup:
    ibz_finalize(&norm_denom);
    ibz_finalize(&degree_resp);
    ibz_finalize(&degree_full_resp);
    ibz_finalize(&remain);
...
static int
sample_response_lattice_ball(response_isogeny_stats_t *stats,
                             const quat_lattice_t *lattice,
                             const ibz_t *lattice_content,
                             uint32_t response_length,
                             ...
                             uint64_t response_samples)
{
    ...
    ibz_t radius, corrected_radius, qf_value, tmp;
    ibz_init(&radius);
    ibz_init(&corrected_radius);
    ibz_init(&qf_value);
    ibz_init(&tmp);
    ...
    response_bound(&radius, lattice_content, response_length);
    quat_lattice_gram(&G, lattice, &QUATALG_PINFTY);

    // Correct as in quat_lattice_sample_from_ball: denominator and twice the norm.
    ibz_mul(&corrected_radius, &radius, &lattice->denom);
    ibz_mul(&corrected_radius, &corrected_radius, &lattice->denom);
    ibz_mul(&corrected_radius, &corrected_radius, &ibz_const_two);

    int nontrivial = quat_lattice_bound_parallelogram(&box, &U, &G, &corrected_radius);
    set_candidate_space(stats, &box);
    if (!nontrivial) {
        ret = 1;
        goto cleanup;
    }

    while (stats->responses < response_samples) {
        int is_odd = 0;

        if (max_candidates != 0 && stats->candidates >= max_candidates) {
            ret = 1;
            goto cleanup;
        }
        ...
        for (int i = 0; i < 4; i++) {
            if (ibz_is_zero(&box[i])) {
                ibz_copy(&y[i], &ibz_const_zero);
            } else {
                ibz_add(&tmp, &box[i], &box[i]);
                if (!ibz_rand_interval(&y[i], &ibz_const_zero, &tmp))
                    goto cleanup;
                ibz_sub(&y[i], &y[i], &box[i]);
            }
        }

        stats->candidates++;
        ibz_mat_4x4_eval_t(&x, &y, &U);
        quat_qf_eval(&qf_value, &G, &x);
        if (ibz_is_zero(&qf_value) || ibz_cmp(&qf_value, &corrected_radius) > 0)
            continue;

        ibz_mat_4x4_eval(&resp_quat.coord, &lattice->basis, &x);
        ibz_copy(&resp_quat.denom, &lattice->denom);
        quat_alg_normalize(&resp_quat);

        stats->responses++;
        if (response_degree_is_odd(&is_odd, &resp_quat, lattice_content)) {
            if (is_odd) {
                stats->odd_degree++;
            } else {
                stats->even_degree++;
...
cleanup:
    ...
    ibz_finalize(&tmp);
    ibz_finalize(&qf_value);
    ibz_finalize(&corrected_radius);
    ibz_finalize(&radius);
    ibz_mat_4x4_finalize(&G);
    ibz_mat_4x4_finalize(&U);
    ibz_vec_4_finalize(&x);
    ibz_vec_4_finalize(&y);
    ibz_vec_4_finalize(&box);
    return ret;
}

static int
enumerate_response_lattice_ball(response_isogeny_stats_t *stats,
                                const quat_lattice_t *lattice,
                                const ibz_t *lattice_content,
                                uint32_t response_length,
                                uint64_t max_candidates,
                                uint64_t max_responses)
{
    ...
    if (max_responses != 0) {
        return sample_response_lattice_ball(
            stats, lattice, lattice_content, response_length, max_candidates, max_responses);
    }

    ibz_vec_4_t box, lower, y, x;
    ibz_vec_4_init(&box);
    ibz_vec_4_init(&lower);
    ibz_vec_4_init(&y);
    ibz_vec_4_init(&x);
    ...
    ibz_t radius, corrected_radius, qf_value;
    ibz_init(&radius);
    ibz_init(&corrected_radius);
    ibz_init(&qf_value);
    ...
    ibz_pow(&radius, &ibz_const_two, response_length);
    ibz_sub(&radius, &radius, &ibz_const_one);
    ibz_mul(&radius, &radius, lattice_content);
    ...
    for (int i = 0; i < 4; i++) {
        ibz_neg(&lower[i], &box[i]);
        ibz_copy(&y[i], &lower[i]);
    }
    ...
    do {
        ...
        if (!ibz_is_zero(&qf_value) && ibz_cmp(&qf_value, &corrected_radius) <= 0) {
            ...
            if (max_responses != 0 && stats->responses >= max_responses) {
                ret = 1;
                goto cleanup;
            }
        }
    } while (increment_box_vector(&y, &lower, &box));
...
int
protocols_response_isogeny_stats(response_isogeny_stats_t *stats,
                                 ...
                                 uint64_t max_responses)
{
    ...
    if (!commit(&E_com, &basis_even_com, &lideal_commit))
        goto cleanup;

    hash_to_challenge(&sig.chall_coeff, pk, &E_com, m, l);
    compute_challenge_ideal_signature(&lideal_chall_two, &sig, sk);
    compute_response_lattice(&lattice_hom_chall_to_com, &lattice_content, sk, &lideal_chall_two, &lideal_commit);

    ret = enumerate_response_lattice_ball(
        stats,
