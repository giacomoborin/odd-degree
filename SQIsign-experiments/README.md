...
## Odd-response experiments

This fork includes an experiment driver for counting odd-degree response
isogenies. There is no separate shell runner; build the experiment binary and
run it directly from the repository root.

```
cmake --build build --target odd_response_experiment_lvl1
mkdir -p experiments
```

Exact level-1 enumeration for 10,000 commitment/challenge pairs:

```
build/apps/odd_response_experiment_lvl1 \
  --iterations=10000 \
  --response-length=126 \
  --max-candidates=0 \
  --max-responses=0 \
  > experiments/lvl1_odd_response_raw.txt
```

Sampled level-1 run at response exponent 156, with 1,000 pairs and 10,000
valid responses per pair:
...
build/apps/odd_response_experiment_lvl1 \
  ...
  --max-responses=10000 \
  > experiments/lvl1_odd_response_e156_1000_raw.txt
```

Summarize or plot the resulting CSV-style output with:

```
python3 scripts/odd_response_histogram.py experiments/lvl1_odd_response_raw.txt \
  --plot experiments/lvl1_odd_response_histogram.png

python3 scripts/odd_response_histogram.py experiments/lvl1_odd_response_e156_1000_raw.txt \
  --plot experiments/lvl1_odd_response_e156_1000_histogram.png
```
